...
{
	vector3 up = glm::normalize(m_v3Up - m_v3Position);
	m_v3Position += (up * a_fDistance);
	m_v3Target += (up * a_fDistance);
	m_v3Up += (up * a_fDistance);
...
void Simplex::MyCamera::CalculateViewMatrix(void)
{
	...
	m_m4View = glm::lookAt(m_v3Position, QuatConversion(m_v3Target), glm::normalize(m_v3Up - m_v3Position));
	m_m4View = rotMatrix *  m_m4View;
...
//	rotQuat = glm::quat(yawAngle, pitchAngle, 0.0f);
	...
	quaternion p = glm::angleAxis(-pitchAngle, AXIS_Y);
	quaternion y = glm::angleAxis(yawAngle, AXIS_X);
	rotQuat = r * p * y;
...
quaternion Simplex::MyCamera::getConjugate() //name is self explanatory
{
